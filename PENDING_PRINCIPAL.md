...
|---|---|---|---|---|---|---|
| PENDING-P-043 | 2026-08-21 | question-banks (+ canon/marklogic · `BAN-S10`) | **HOW MANY কাল DOES C5 TEACH, AND IS `BAN-S10 C5` OWED A `taught_set` THE WAY `BAN-S11` IS?** `BAN-S10` at C5 is UNSELECTED (CD-181): all three of `ভাষারীতি পরিবর্তন` · `পদ নির্ণয়` · `ক্রিয়ার কাল` are admitted and none is off-choice. The register names the FORM and says nothing about its GRANULARITY — no `taught_set`, no sub-type list, authority NAPE §১০. পাঠ ২২ wave 2 authored five `ক্রিয়ার কাল` items keying `সাধারণ বর্তমান` · `সাধারণ অতীত` · `ঘটমান অতীত` · `পুরাঘটিত অতীত`, i.e. the four-way-per-tense division. **REVIEW `REV-QB-BAN-C5-U22-w2` W2-03 found that nobody has ruled C5 teaches that**, and cited `BAN-S11`'s precedent against it: that row carries an explicit `taught_set` **because** the spine names no marks, and `CD-165`'s standing rule is *ABSENCE READS AS NOT DECLARED, NEVER AS PERMISSIVE*. On the same reading `BAN-S10`'s silence about কাল granularity is not permission to pick one. **A second, sharper defect rode on the first (W2-02):** two items keyed the same present-tense form two ways — `চায়` as `সাধারণ বর্তমান`, `নেমে আসে` as `সাধারণ অতীত` — because one keyed by verb FORM and the other by narrative SENSE, the chapter being written in the historical present throughout. A student applying either rule consistently is wrong on one of the two, and the disagreement is written into the bank rather than discovered at marking. **This reaches every remaining chapter, not পাঠ ২২:** `BAN-S10` runs C2→C5 and `ক্রিয়ার কাল` is admitted at each, so whatever is ruled here keys eighteen more C5 Bangla banks and the C2–C4 ladder beneath them. | **`ক্রিয়ার কাল` IS NOT AUTHORED AT C5 UNTIL THIS IS RULED.** Principal ruling 2026-08-21, taken on the wave-3 pass: the five items were DROPPED rather than re-keyed, and `S10` draws `পদ নির্ণয়` only — five items, one for each of the five পদ C5 teaches (বিশেষ্য · সর্বনাম · বিশেষণ · ক্রিয়া · অব্যয়). **The reason for dropping rather than ruling is itself the default:** settling what C5's grammar syllabus contains in order to close one bank is deciding a curriculum question for the wrong reason, and `CD-171` leaves no pool floor that a short slot could violate, so nothing was owed. `পদ নির্ণয়` remains fully authorable meanwhile; the slot is not blocked, only one of its three admitted forms is. | **Before the next chapter that reaches a verb — পাঠ ২১ or ২৩, whichever opens first.** Not blocking পাঠ ২২, which shipped at `e7e986f` with `S10` at five `পদ নির্ণয়` items and repo-wide `RUNALL_SENTINEL=CLEAN`. **Evidence the ruling should be taken against, not from this row:** the NCTB Class 5 বাংলা ব্যাকরণ syllabus itself, and what C2–C4's own `BAN-S10` rows and existing banks already do — neither was read for this row and neither is asserted here (`QB-CR-018`). If the answer is three-way, a `taught_set` field at `BAN-S10 C5` would carry it the way `CD-165`/`CD-166` carried the punctuation marks at `BAN-S11`, and that is a register change, not a bank change. | OPEN |
| PENDING-P-042 | 2026-08-18 | question-banks (tooling · `ENVELOPE-SYNC`) | **`ENVELOPE-SYNC` DECLARES A NEVER-EXPORTED BANK IN SYNC, AND PRINTS A FALSE SENTENCE DOING IT — the gate built to catch a stale export cannot see an ABSENT one.** Reproduced on all three of পাঠ ১৭ · ১৮ · ১৯ before their exports were built: `RESULT: CLEAN (0 failures)` with the line **`export in sync: 0 envelope(s) == 78 bank item(s)`**. **Cause, read at source: `gates.py:1822` — `if not array.exists() and not single.exists():`.** The honest *no export exists … Reported, not passed* branch requires **BOTH** to be absent, but **`banks/envelopes/single/` is SHARED BY EVERY BANK** and has been populated since U13. For any new bank in a repo that has ever exported, that branch is **unreachable**: both id sets come back empty, both per-id loops `continue` over nothing, no error is raised, and `if not errs` declares sync. **The proposed fix, stated so the work is mechanical rather than exploratory: scope BOTH id sets to the bank's own qid prefix, and REPORT on total absence** — the prefix scoping already exists at `envelope_prefix()` and is used for `single/` but not as an emptiness test. **And the selftest is the second half of the finding: all EIGHT seeds test drift BETWEEN ARTIFACTS THAT EXIST** (export behind, orphan survival, wrapper miscount, wrapper behind, wrapper carrying a retired item, wrapper describing another bank, in-sync quiet, no-`.batch.json` quiet) — **none tests absence.** A ninth seed is owed with the fix. | **Hand-count at fan-in, not the gate's PASS.** Every export is counted by hand — array length, `single/` file count, `batch.item_count` — each against the bank's own item total, and a mismatch stops the run. Verified this session for U17 78·78·78·78, U18 79·79·79·79, U19 46·46·46·46. | before the next bank is authored, and in any case before any session relies on `ENVELOPE-SYNC` to prove an export exists | **CLOSED 2026-08-20.** **Original status:** **OPEN — Principal-owed, and DELIBERATELY NOT BUILT this session by instruction (2026-08-18, R3).** Nothing is blocked: the three banks of this session were exported and hand-counted, and their `ENVELOPE-SYNC` runs now compare real artifacts. **What is blocked is the inference** — a green `ENVELOPE-SYNC` is not evidence that a bank HAS an export, only that any export it has agrees with it. **This is the sixth instance of the session's recurring family** (`QB-CR-017` · `CD-166(c)` · `CD-168` · `CD-170(e)` · `CD-171(e)` · `CD-172(e)`): **a check aimed at a narrower surface than the claim it tests** — here, at drift between artifacts rather than at the existence of one. It was found by three independent workers reaching it separately, and confirmed by the orchestrator reading the branch condition, not by the gate. **CLOSED 2026-08-20 -> TOOLS-CR-018 (built) . TOOLS-CR-023 (record). Both halves discharged, BUT NOT AT THE BRANCH THIS ROW NAMED.** (a) The emptiness test moved to the PREFIX-SCOPED ID SETS, not to `gates.py:1853`: `sing` is globbed `prefix + "*.json"` at 1876 and `arr` fills only from this bank own array file, so `if not (sing or arr)` at 2003 is genuinely bank-scoped and the false green is gone. **(b) `1853` `not array.exists() and not single.exists()` REMAINS AND REMAINS UNREACHABLE** - dead code whose *Reported, not passed* text is superseded by 2004 `NOT COMPARED`. Recorded rather than removed: this row whole finding was that branch, and a future reader who greps for it must find the correction beside it. (c) The **ninth seed is present** - `ok_noexport` at 3626 asserts a no-export bank prints `NOT COMPARED` and does NOT print `in sync`, with `ok_stillsync` at 3637 holding the healthy case. All three facts verified at source 2026-08-20. |
